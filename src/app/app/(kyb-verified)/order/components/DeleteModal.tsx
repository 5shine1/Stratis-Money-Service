...
const DeleteModal: React.FC<Props> = ({ isOpen, onClose, onNext }) => {
  ...
  return (
    <Modal
     ...
     className="relative z-50 g-box-back w-full max-w-400  rounded-20 shadow-md m-auto border border-modal-border"
    ...
    >
      ...
      <div className="p-12 py-24 md:p-32 bg-transparent  flex flex-col gap-24  items-center">
        ...
        <button
            onClick={onNext}
            className="text-button-text text-16 font-semibold px-48 py-12 rounded-12 gap-8 flex items-center justify-center border border-button-border bg-gradient-to-r from-button-from/10 to-button-to/10 transition-all duration-300 hover:from-button-from/50 hover:to-button-to/50"
          >
          {role === ROLES.ADMIN
